const experienceYear = '+9';

export const AppData = {
  name: 'Mostafa Gholami',
  email: 'mostafagholamidev@gmail.com',
  phone: '+989365895522',
  address: 'Isfahan, Iran',
  nickname: 'mst-ghi',
  birthday: 'April 2, 1993',
  headline: 'Senior Full-Stack Developer',
  resumes: [
    {
      lang: 'en' as const,
      label: 'CV EN',
      shortLabel: 'EN',
      href: '/Mostafa-Gholami-CV-EN.pdf',
      filename: 'Mostafa-Gholami-CV-EN.pdf',
      ariaLabel: 'Download English CV',
    },
    {
      lang: 'fa' as const,
      label: 'CV FA',
      shortLabel: 'FA',
      href: '/Mostafa-Gholami-CV-FA.pdf',
      filename: 'Mostafa-Gholami-CV-FA.pdf',
      ariaLabel: 'Download Persian CV',
    },
  ],
  description: `Senior Full-Stack Developer with ${experienceYear} years of experience designing and building scalable web platforms with NestJS, Next.js, Golang, and Turborepo. Specialized in monorepo architectures, multi-tenant systems, real-time features, and clean, maintainable codebases — with mobile experience in React Native. Delivering complex products from architecture to production for startups and established companies.`,
  experienceYear,
  site: 'https://mst-ghi.github.io/',
  headlineStacks: [
    'Node.js · NestJS · TypeORM · Prisma · Socket.IO',
    'Golang · Gin · Gorm · Middlewares · Scheduler',
    'React · Next.js · Turborepo · React Query · Zustand',
    'Docker · Redis · Kafka · Microservices · Queues',
    'Clean Code · Atomic Design · Type-safe Development',
  ],
  stats: [
    { value: '9+', label: 'Years of Experience' },
    { value: '20+', label: 'Projects Delivered' },
    { value: '10+', label: 'Companies & Teams' },
    { value: '2', label: 'Open Source Hubs' },
  ],
  education: {
    school: 'Islamic Azad University',
    degree: 'Bachelor in Computer Engineering',
  },
  languages: [
    { name: 'Persian', level: 'Native' },
    { name: 'English', level: 'B1 — Intermediate' },
  ],
  socials: {
    github: 'https://github.com/mst-ghi',
    linkedin: 'https://www.linkedin.com/in/mst-ghi/',
    telegram: 'https://t.me/mostafa_ghi',
  },
};
